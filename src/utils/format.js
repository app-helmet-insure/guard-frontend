...
export const formatAmount = (value, decimals = 18, fixed = 6) => new BigNumber(
  fromWei(value, decimals).toFixed(fixed === -1 ? null : fixed, 1)
)
  .toNumber()
  .toString()

export const numToWei = (value, decimals = 18) => new BigNumber(
  toWei(value, decimals)
).toString()

export const fromWei = (value, decimals = 18) => new BigNumber(value).dividedBy(new BigNumber(10).pow(decimals))

const toWei = (value, decimals) => new BigNumber(value).multipliedBy(new BigNumber(10).pow(decimals))

export const weiPlus = (value1, value2) => new BigNumber(
